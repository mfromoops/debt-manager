...
import { an as REDIRECT_STATUS_CODES, A as AstroError, ao as ActionsReturnedInvalidDataError, ap as escape, D as DEFAULT_404_COMPONENT } from './astro/server_KeCo5y3E.mjs';
...
const __vite_import_meta_env__ = {"ASSETS_PREFIX": undefined, "BASE_URL": "/", "DEV": false, "MODE": "production", "PROD": true, "SITE": "https://debt-manager.app", "SSR": true};
...
function serializeActionResult(res) {
  if (res.error) {
    if (Object.assign(__vite_import_meta_env__, { _: process.env._, status: process.env.status, name: process.env.name })?.DEV) {
...
function deserializeActionResult(res) {
  if (res.type === "error") {
    ...
    if (Object.assign(__vite_import_meta_env__, { _: process.env._, status: process.env.status, name: process.env.name })?.PROD) {
